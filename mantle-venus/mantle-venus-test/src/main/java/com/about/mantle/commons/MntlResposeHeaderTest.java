...
public abstract class MntlResposeHeaderTest extends MntlVenusTest {
    ...
    public String getHeaderValue(WebResponse response, String name)
    {
        return response.getResponseHeaders().stream()
                .filter(item -> item.getName().equals(name)).findFirst().get().getValue();
    }
}
